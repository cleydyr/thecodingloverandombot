  ...
  constructor(telegramToken) {
	...
	processPost(msg) {
		return post => {
			...
			if (imgURL.endsWith(".gif")) {
				...
				this.sendMessage(msg.chat.id, "Sorry. We have faced an internal error. Please try again.");
			}
		};
	}
	...
	sendGif(msg, imgURL, title) {
		this.bot.sendDocument(msg.chat.id, imgURL, {
			...
			.catch(err => {
				console.log(err);
				this.sendMessage(msg.chat.id, "Sorry. We couldn't find a gif. Please try again.");
			});
	}
